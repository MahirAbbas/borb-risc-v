...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run an existing ELF directly on borb-sim with timestamped repo-local artifacts.")
    ap.add_argument("elf", help="Existing ELF to run")
    ap.add_argument("--name", default="", help="Override test name (default: ELF stem)")
    ap.add_argument("--out-root", default="verif/directed/out_elf", help="Output directory root")
    ap.add_argument("--sim", default="verif/riscof/borb/build/obj_dir/VSoC", help="Path to borb simulator binary")
    ...
    ap.add_argument("--xlen", type=int, default=64, choices=[32, 64], help="XLEN/toolchain width for nm/objdump")
    ...
    if not elf.exists():
        raise SystemExit(f"elf file not found: {elf}")

    sim = Path(args.sim).resolve()
    sim_make_dir = Path(args.sim_make_dir).resolve()
    out_root = Path(args.out_root).resolve()
    test_name = args.name if args.name else elf.stem
