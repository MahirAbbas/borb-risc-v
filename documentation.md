...
- Milestone 31 has started the central issue contract: `BackendPipe.select(...)` maps decoded work onto `ALU0/ALU1/Branch/MulDiv/LoadStore/FALU/FMAC/CSRTrap/Vector`, and lane 0 now carries `BackendIssue.SELECTED_PIPE` from the current dispatch/issue boundary. Lane 1 uses the same selector with ALU1 preference. Current pairing restrictions are intentionally unchanged until the physical retiming and broader pipe conflict logic are in place.
