...
- The active roadmap is `agent/plan4.md`. It supersedes the older RV64GC-only widening plan and separates the frozen `current` subset from the full RVA23S64 target.
- Hard gates for every milestone are full ACT4 and CoreMark:
  - `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1`
  - `./run_coremark.sh`
