...
object test_hazards_app extends App {
  SimConfig.withWave.compile(new test_hazards()).doSim { dut =>
    ...
    for (i <- 0 until 31) {
      dut.coreArea.srcPlugin.regfileread.regfile.mem
        .setBigInt(i, BigInt("0", 10))
    }
    ...
    val instructions: List[(Long, BigInt)] = List(
      ...
      (
        28,
        BigInt("0053c433", 16)
      ), // xor  x8, x7, x5         # DEPENDS on x7 # x8 = 5
      ...
      (
        44,
        BigInt("00558633", 16)
      ), // add  x12, x11, x5       # chain dependency
      ...
      (
        52,
        BigInt("00c68733", 16)
      ), // add  x14, x13, x12      # depends only on x12
      ...
      (
        64,
        BigInt("003808b3", 16)
      ), // add  x17, x16, x3       # uses x16 immediately
      (
        68,
        BigInt("00488933", 16)
      ), // add  x18, x17, x4       # uses x17 immediately
      (
        72,
        BigInt("005909b3", 16)
      ), // add  x19, x18, x5       # uses x18 immediately
      ...
      (
        80,
        BigInt("00418ab3", 16)
      ), // add  x21, x3, x4        # unrelated, should not stall
      (
        84,
        BigInt("015a0b33", 16)
      ) // add  x22, x20, x21      # depends on x20, tests correct stall or bypass timing
    ...
    for (i <- 0 to 65) {
      ...
      println(
        s"RESULT.valid ${dut.coreArea.readHere.valid_result.toBoolean}, RESULT.address ${dut.coreArea.readHere.rdaddr.toLong}, RESULT.data ${dut.coreArea.readHere.result.toLong}, IMMED: ${dut.coreArea.readHere.immed.toLong},SENDTOALU: ${dut.coreArea.readHere.sendtoalu.toBoolean}, VALID: ${dut.coreArea.readHere.valid.toBoolean} "
      )
