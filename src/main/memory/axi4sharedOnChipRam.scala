...
object Axi4SharedOnChipRam {
  ...
  def main(args: Array[String]) {
    SpinalVhdl(
      new Axi4SharedOnChipRam(32, 1024, 4).setDefinitionName("TopLevel")
    )
  }
}

case class Axi4SharedOnChipRam(
    dataWidth: Int,
    byteCount: BigInt,
    idWidth: Int,
  ...
  val axiConfig =
    Axi4SharedOnChipRam.getAxiConfig(dataWidth, byteCount, idWidth)
...
case class Axi4SharedOnChipRamPort(config: Axi4Config)
    extends ImplicitArea[Axi4Shared] {
...
class Axi4SharedOnChipRamMultiPort(
    config: Axi4Config,
    wordCount: BigInt,
    portCount: Int
) extends Component {
  ...
  if (
    ...
    SpinalWarning(
      "Axi4SharedOnChipRamMultiPort might not support Axi4 Lock, Region, Cahce, Prot and Qos featrues!"
    )
