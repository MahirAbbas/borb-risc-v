...
import borb.execute.fpu.{FpuAddSub, FpuDivSqrt, FpuMul, FpuScalarMisc, FpuSoftFloatUtils}
    ...
    val trapLogic = new execStage.Area {
      ...
      when(faddsubSFire) {
        ...
        val addSub = FpuAddSub.addSubS(
          fpRegs(rs1)(31 downto 0),
          fpRegs(rs2)(31 downto 0),
          fcvtRm,
          isFsubSInsn
        )
        fpRegs(rd) := B(BigInt("FFFFFFFF", 16), 32 bits) ## addSub.data
      ...
      when(fmulSFire) {
        ...
        fpRegs(rd) := B(BigInt("FFFFFFFF", 16), 32 bits) ## mul.data
      ...
      when(fdivSFire) {
        ...
        fpRegs(rd) := B(BigInt("FFFFFFFF", 16), 32 bits) ## div.data
      ...
      when(fsqrtSFire) {
        ...
        fpRegs(rd) := B(BigInt("FFFFFFFF", 16), 32 bits) ## sqrt.data
      ...
      when(fclassSFire) {
        val rs1 = insn(19 downto 15).asUInt
        val cls = FpuScalarMisc.classifyS(fpRegs(rs1)(31 downto 0))
      ...
      when(fminmaxSFire) {
        ...
        fpRegs(rd) := B(BigInt("FFFFFFFF", 16), 32 bits) ## minMax.data
      }
