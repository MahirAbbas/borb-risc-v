...
- single-core only
- `ACT4 + CoreMark` are the only hard gates
...
- Validator-supported scalar extension surface declared in `verif/act4/borb-rva23s64/test_config.yaml`: `Zba`, `Zbb`, `Zbs`, `Zca`, `Zcb`, `Zcmop`, `Zfa`, `Zfh` as the local tooling stand-in for the verified `Zfhmin`/half-memory slice, `Zicbom`, `Zicbop`, `Zicboz`, `Zicond`, `Zicntr`, `Zihintpause`, `Zihpm`, `Zimop`, and `Svnapot`.
- Supervisor/VM behavior covered by the local directed and ACT4 gates for the current single-core floor, including direct-mode `stvec`, `stval`, `scounteren`, RV64 UXL, fault-on-clear-A/D Sv39 behavior, legal `Svinval` encodings, and the verified level-0 64 KiB `Svnapot` leaf slice.
- In-order scalar backend with conservative asymmetric dual-issue support for selected integer/branch pairings.
- ACT4 and CoreMark as the only hard gates for this program.
...
- Focused ACT4 subsets for newly claimed scalar extension slices.
- Directed assembly tests for weak ACT4 coverage, cache-block behavior, CSR/trap behavior, memory-model corner cases, and vector slices.
...
- Consolidated the milestone verification checklist around the two hard gates: full ACT4 and CoreMark.
...
- Add focused directed tests where current ACT4 coverage is weak.

Acceptance:
- declared scalar ISA includes the required `B` surface actually implemented
- full scalar ACT4 stays green
...
- Declared `RV64IMAFDCSUZicsr_Zifencei_Zba_Zbb_Zbs` in the canonical ACT4 ISA YAML without claiming the legacy umbrella `B` or non-mandatory `Zbc`.
...
- Historical note: the old Spike plugin was fixed so reference runs included `_Zba_Zbb_Zbs` when the ISA YAML declared those extensions; the first focused subset failure was a reference-model ISA mismatch, not a DUT ALU divergence.
- Validation passed:
  - `sbt compile`
  - `./run_directed.sh verif/directed/asm/rv64_zba_zbb_zbs_smoke.S --march rv64gc_zba_zbb_zbs_zicsr_zifencei --trace-commit`
  - focused 40-test ACT4 subset for `Zba/Zbb/Zbs`
  - `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1338/1338`, zero generated failure reports)
  - `./run_coremark.sh`
...
- Declared `RV64IMAFDCSUZicond_Zicsr_Zifencei_Zihintpause_Zimop_Zfa_Zfh_Zca_Zcb_Zcmop_Zba_Zbb_Zbs` in the canonical ACT4 ISA YAML and propagated the same validator-supported multi-letter user extensions into the local Spike reference `--isa`.
...
- Aligned the borb ACT4 model header signature layout with the Spike reference header so NOP-like tests that store signature pointer values do not compare different linker-layout addresses.
- Added `Zfa` scalar FP decode and execution support for `fli.{s,d}`, minimum/maximum magnitude, quiet compares, round-to-integer, and `fcvtmod.w.d`; the focused local ACT4 `Zfa` subset passes.
- Added the `Zfhmin` half conversion subset for `fcvt.{l,lu}.h` and `fcvt.h.{l,lu}`. The repo's riscv-config version does not understand the narrower `Zfhmin` spelling and requires `Zfh` when `Zfa` is declared, so full ACT4 filtering keeps full half-precision arithmetic out of the claimable surface.
...
- focused ACT4 subset for `czero.eqz-01.S,czero.nez-01.S` (`2/2`, zero generated failure reports)
- `./run_directed.sh verif/directed/asm/rv64_zimop_smoke.S --march rv64gc_zimop_zicsr_zifencei --trace-commit`
- focused 40-test ACT4 subset for `mop.r.*` / `mop.rr.*` (`40/40`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1338/1338`, zero generated failure reports)
- `./run_coremark.sh`
- `./run_directed.sh verif/directed/asm/rv64_zcmop_smoke.S --march rv64gc_zcmop_zicsr_zifencei --trace-commit`
- focused 8-test ACT4 subset for `c.mop.*` (`8/8`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1338/1338`, zero generated failure reports, after `Zcmop` and ACT4 header layout fix)
- `./run_coremark.sh`
- focused 12-test ACT4 subset for `Zcb` compressed byte/halfword load/store, extend, not, and multiply operations (`12/12`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1338/1338`, zero generated failure reports, after `Zcb`)
- `./run_coremark.sh` (`PASS`, tohost `0x1`, after `Zcb`)
- focused 78-test ACT4 subset for `Zfa` (`78/78`, zero generated failure reports)
- focused 18-test ACT4 subset for `Zfhmin` half conversion (`18/18`, zero generated failure reports)
...
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1539/1539`, zero generated failure reports, after final `Zfh` memory and PMP hardening)
...
- ACT4/direct counter tests pass
- CoreMark perf reporting still works

Implementation Notes:
- Declared `Zicntr` and `Zihpm` in the canonical ACT4 ISA YAML and propagated both extensions into the local Spike reference `--isa` string.
...
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1539/1539`, zero generated failure reports)
...
- full scalar ACT4 remains green

Implementation Notes:
- Declared validator-supported cache-block extensions `Zicbom`, `Zicbop`, and `Zicboz` in the canonical ACT4 ISA YAML and propagated the cache-block/memory-profile extension names into the local Spike reference `--isa` builder.
...
- `make -C verif/borb-sim/sim clean all`
...
- full scalar ACT4 gate remains green

Implementation Notes:
- Declared validator-supported `Svnapot` in the canonical ACT4 ISA YAML and propagated it into the local Spike `--isa` builder.
...
- Kept `Svade` as fault-on-clear-A/D behavior: page-table accesses require `A=1`, and writes require `D=1`, which is covered by the active Sv39 ACT4 A/D tests.
- Kept `Ssccptr` backed by the existing VM/PMP page-table access path: page-table reads are explicit memory-side accesses and the active `vm_pmp_on_pte` ACT4 tests remain green.
...
- Filtered `vm_reserved_svnapot_S_mode.S` out of full ACT4 once `Svnapot` is declared; that test is explicitly a negative test for implementations without Svnapot support. The wrapper now clears stale excluded-test debug artifacts before generating failure reports.
...
- focused VM ACT4 subset for `vm_reserved_svnapot_S_mode.S,vm_A_and_D_S_mode.S,vm_reserved_svpbmt_S_mode.S` (`3/3`, zero generated failure reports, before excluding the obsolete Svnapot-negative test from full runs)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1538/1538`, zero generated failure reports)
- `./run_coremark.sh` (`PASS`, tohost `0x1`)
...
- focused AMO ACT4 subset for `amoadd.w-01.S,amomin.d-01.S,amomaxu.d-01.S,amoswap.d-01.S` (`4/4`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1538/1538`, zero generated failure reports)
- `./run_coremark.sh` (`PASS`, tohost `0x1`)
...
- The current PTW remains the existing shadow-backed Sv39 access path, now fronted by explicit TLB structures. It is not yet a fully independent memory-walking FSM; that remains the boundary to improve if the roadmap later demands platform page-table walking outside the ACT4 shadow model.

Validation:
- `sbt compile`
- `sbt "runMain borb.SoC"`
- `make -C verif/borb-sim/sim clean all`
- focused VM ACT4 subset for `vm_A_and_D_S_mode.S,vm_satp_access_tests.S,vm_mxr_S_mode.S,sv39_pmp_on_pte_S_mode.S` (`4/4`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1538/1538`, zero generated failure reports)
...
- Kept the 32 KiB L1I floor from Milestone 19 and held outstanding fetch misses at the proven 2-entry depth. The attempted 4-entry miss floor exposed stale refill/redirect races in branch-heavy ACT4 tests, so that performance step is deferred rather than shipped red.
...
- `sbt "runMain borb.SoC"`
- `make -C verif/borb-sim/sim clean all`
- focused branch/C ACT4 subset for `beq-01.S,bne-01.S,blt-01.S,bge-01.S,bltu-01.S,bgeu-01.S,cbeqz-01.S,cbnez-01.S` (`8/8`, zero generated failure reports)
- `./run_act4.sh --profile rva23s64-full --verilate-jobs 10 --sim-jobs 10 --sim-threads 1 --fast-sim` (`1538/1538`, zero generated failure reports)
...
Implementation Notes:
- Audited `verif/act4/borb-rva23s64/test_config.yaml` against the implemented and locally verified surface. The declaration remains scalar/profile-support focused and intentionally does not advertise full base `V`, vector profile support, `Zvbb`, or `Zvkt`.
...
- differential tests for RVV slices where ACT4 is weak or absent
