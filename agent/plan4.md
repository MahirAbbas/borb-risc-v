...
The target is not “make the current 1540-test RISCOF run pass.” The target is a profile-complete borb implementation and verification environment that can honestly claim full mandatory RVA23S64 support.

That requires four parallel bodies of work:

0. Expand RISCOF and the RISC-V architectural suite coverage to the full RVA23S64 profile.
...
- Record the current passing subset ISA, full RISCOF result, CoreMark cycles, known waivers, and known non-claims.
- Preserve `--profile current` as the regression floor.
- Acceptance:
  - current RISCOF passes unchanged;
...
- Each row must include: extension, profile source, implementation owner area, declaration status, RISCOF coverage, directed coverage, reference-model support, and current blocker.
- Acceptance:
...
- Define what “done” means for a mandatory feature:
  ...
  - covered by RISCOF or documented backfill tests;
  - included in final docs.
- Acceptance:
  - a feature cannot be marked complete without evidence links.

## Phase 1: Expand RISCOF Into A Profile-Scale Harness

### M42: Add RISCOF Profile Modes
...
### M44: Expand The RISC-V RISCOF Suite To RVA23S64
...
- Acceptance:
  - final RISCOF output cannot hide missing mandatory coverage behind a pass count.
...
- Acceptance:
  - every RISCOF failure directly points to the responsible profile feature.
...
- Acceptance:
  - RISCOF atomic tests and directed reservation tests pass.
...
- Acceptance:
  - each extension has either RISCOF coverage or directed backfill.
...
- Acceptance:
  - FP RISCOF subsets and rounding/flag directed tests pass.
...
- Acceptance:
  - misaligned RISCOF and directed tests pass.
...
- Acceptance:
  - docs match ledger and final RISCOF output.

### M101: Full RVA23S64 RISCOF Signoff

- Run the final full profile:
  - `./run_riscof.sh --profile rva23s64-full --fast-sim --verilate-jobs 10 --sim-jobs 10 --sim-threads 1`
- Acceptance:
...
- Acceptance:
  - no RISCOF, directed, or CoreMark regression.
...
- `./run_riscof.sh --profile current --fast-sim --verilate-jobs 10 --sim-jobs 10 --sim-threads 1`
- `./run_riscof.sh --profile rva23s64-full --fast-sim --verilate-jobs 10 --sim-jobs 10 --sim-threads 1`
- `./run_tenstorrent.sh`
- `./run_formal.sh`
- full directed suite
...
- Do not count a RISCOF pass as full profile coverage unless missing/blocked coverage is zero.
